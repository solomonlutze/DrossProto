...
// Ruthlessly edited from the LineBrush scriptable brush example in unity's 2dTilemap Extras
...
namespace UnityEditor.Tilemaps
{
  ...
  /// </summary>
  [CustomGridBrush(true, false, false, "Hex Line Brush")]
  [CreateAssetMenu(fileName = "New Hex Line Brush", menuName = "Brushes/Hex Line Brush")]
  public class HexLineBrush : LayerFloorBrush
  {
    /// <summary>
    ...
    public override void Paint(GridLayout grid, GameObject brushTarget, Vector3Int position)
    {
      if (lineStartActive)
      {
        FloorLayer layer = WorldObject.GetFloorLayerOfGameObject(brushTarget.gameObject); // Only used bc my line algorithm is real specific
        TileLocation startLoc = new TileLocation(lineStart.x, lineStart.y, layer);
        TileLocation endLoc = new TileLocation(position.x, position.y, layer);
        if (startLoc == endLoc)
          base.Paint(grid, brushTarget, position);
        else
        {
          foreach (TileLocation tile in PathfindingSystem.GetTilesAlongLine(startLoc, endLoc))
          // foreach (var point in LineBrush.GetPointsOnLine(startPos, endPos, lineBrush.fillGaps))
          {
            Vector3Int paintPos = new Vector3Int(tile.x, tile.y, position.z);
  ...
  /// </summary>
  [CustomEditor(typeof(HexLineBrush))]
  public class HexLineBrushEditor : GridBrushEditor
  {
    private HexLineBrush lineBrush { get { return target as HexLineBrush; } }
    ...
    public override void OnPaintSceneGUI(GridLayout grid, GameObject brushTarget, BoundsInt position, GridBrushBase.Tool tool, bool executing)
    {
      ...
      if (lineBrush.lineStartActive)
      {
        ...
        FloorLayer layer = WorldObject.GetFloorLayerOfGameObject(brushTarget.gameObject); // Only used bc my line algorithm is real specific
        TileLocation startLoc = new TileLocation(lineBrush.lineStart.x, lineBrush.lineStart.y, layer);
        TileLocation endLoc = new TileLocation(position.x, position.y, layer);
        if (startLoc == endLoc)
