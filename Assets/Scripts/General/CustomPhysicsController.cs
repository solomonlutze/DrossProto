...
public class CustomPhysicsController : MonoBehaviour {
	...
	// FIXME: This shouldn't be permanent! It only exists so swarmlings - themselves a test behavior - can
	// set acceleration on thier CPC. Get rid of it!!!
	public void TRASHY_INIT(float acceleration) {
		...
		foreach (Collider2D col in cols) {
			if (!col.isTrigger) {
				...
				for (int i = 0; i < 20 - hits && i < myHits; i++) {
					...
					if (res[i].collider.GetComponentInChildren<Tilemap>() != null) { // TODO: this is... probably not great
