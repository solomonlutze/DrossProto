...
public class Weapon : MonoBehaviour {
	...
	public void CreateHitbox(string hitboxId) {
		HitboxInfo hbi = weaponData.attackAnimations[currentComboLength - 1].hitboxes.Find(x => x.id == hitboxId);
		if (hbi == null) { Debug.LogError("Animation "+weaponData.attackAnimations[currentComboLength - 1].attackAnimation+" has no hitbox with id "+hitboxId); }
 		Hitbox hb = GameObject.Instantiate(weaponData.hitboxPrefab, spawnTransform.position, spawnTransform.rotation) as Hitbox;
