...
public class SuperTextMeshEditor : Editor
{
    override public void OnInspectorGUI()
    {
        ...
        if (GUI.Button(new Rect(r.width - 2, r.y, 16, 16), new GUIContent("", "Edit TextData"), textDataStyle))
        { //place at exact spot
        ...
        {//show textdata file instead
            ...
            if (stm.data.showTextColorFoldout)
            {
                ...
                {
                    //Gather all data:
                    foreach (KeyValuePair<string, STMGradientData> i in stm.data.gradients.OrderBy(x => -x.Value.GetInstanceID()))
                    { //reorder so the order stays consistent
                        EditorGUI.indentLevel++;
                        i.Value.showFoldout = EditorGUILayout.Foldout(i.Value.showFoldout, i.Key, foldoutStyle);
                        EditorGUI.indentLevel--;
                        if (i.Value.showFoldout) if (i.Value != null) i.Value.DrawCustomInspector(stm); //draw a custom inspector for each Jitter
        ...
        { //draw actual text mesh inspector:
            ...
            if (stm.showAppearanceFoldout)
            {
                ...
                if (stm.bestFit == SuperTextMesh.BestFitMode.Always)
                { //no editing value
                ...
                if (stm.textMaterial != null)
                {
                    ...
                    if (stm.showMaterialFoldout)
                    { //show shader settings
            ...
            if (stm.showPositionFoldout)
            {
                ...
                EditorGUILayout.Space(); //////////////////SPACE
                if (!stm.uiMode)
                { //wrapping text works differently for UI:
  ...
  //foldout bools for editor. not on the GUI script, cause they get forgotten
    ...
    */
    ...
    void OnApplicationFocus(bool focused)
    {
        ...
        { //only care if the application cares
	...
	*/
    ...
    public void Rebuild(float startTime, bool readAutomatically)
    {
        ...
        RebuildTextInfo();
        if (audioSource != null)
        {//initialize an audio source, if there's one. these settings are needed to behave properly
    ...
    { //update data needed for pre-existing mesh
        ...
        for (int i = 0, iL = hyphenedText.Length; i < iL; i++)
        {
            ...
            if (info[i].drawAnimData.startColor != Color.clear)
	...
	*/
    string SpecialKeyToName(char ch)
    { //since you can't put these in filenames
    ...
    void PlaySound(int i)
    {
        ...
        {//audio stuff
            if (info[i].stopPreviousSound || !audioSource.isPlaying)
            {
                ...
                else if (info[i].audioClipData != null)
    ...
    {//return a cleaned up string and update textinfo!
        ...
        { //for each character to parse thru,
            ...
            { //no other delay yet...? /hasnt checkedAgain yet
                ...
                { //only if next character is "free". So strings like "Oh......... okay." only see the last delay on periods!
                ...
                {//don't do anything if there's no closing tag at all
                    ...
                    switch (myTag)
                    {
                        ...
                        case "<c=":
                            ...
                            if (data.textures.ContainsKey(myString))
                            {// is this a texture?
                            ...
                            else
                            {
                                ...
                                if (data.gradients.ContainsKey(myString))
                                { //is this a gradient?
                        ...
                        case "<s=":
                            float thisSize;
                            if (float.TryParse(myString, System.Globalization.NumberStyles.Any, System.Globalization.CultureInfo.InvariantCulture, out thisSize))
                            { //parse as a float
                        ...
                        case "<size=":
                            float thisSize2;
                            if (float.TryParse(myString, System.Globalization.NumberStyles.Any, System.Globalization.CultureInfo.InvariantCulture, out thisSize2))
                            { //parse as a float
                        ...
                        //Delay
                        case "<d=":
                            if (data.delays.ContainsKey(myString))
                            ...
                            { //see if it's an integer
                                int thisDelay2;
                                if (int.TryParse(myString, out thisDelay2))
                                { //parse as an int
                        ...
                        case "<t=":
                            float thisTiming;
                            if (float.TryParse(myString, System.Globalization.NumberStyles.Any, System.Globalization.CultureInfo.InvariantCulture, out thisTiming))
                            { //parse as a float
                        ...
                        //Waves
                        case "<w=":
                            if (data.waves.ContainsKey(myString))
                            { //is it a preset?
                        ...
                        //Jitters
                        case "<j=":
                            if (data.jitters.ContainsKey(myString))
                            { //is it a preset?
                        ...
                        case "<overridePitch=":
                            ...
                            if (float.TryParse(myString, System.Globalization.NumberStyles.Any, System.Globalization.CultureInfo.InvariantCulture, out thisOverridePitch))
                            { //parse as a float
                        ...
                        case "<minPitch=":
                            float thisMinPitch;
                            if (float.TryParse(myString, System.Globalization.NumberStyles.Any, System.Globalization.CultureInfo.InvariantCulture, out thisMinPitch))
                            { //parse as a float
                        ...
                        case "<maxPitch=":
                            float thisMaxPitch;
                            if (float.TryParse(myString, System.Globalization.NumberStyles.Any, System.Globalization.CultureInfo.InvariantCulture, out thisMaxPitch))
                            { //parse as a float
                        ...
                        case "<speedReadPitch=":
                            ...
                            if (float.TryParse(myString, System.Globalization.NumberStyles.Any, System.Globalization.CultureInfo.InvariantCulture, out thisSpeedReadPitch))
                            { //parse as a float
                        ...
                        case "<readDelay=":
                            float thisReadDelay;
                            if (float.TryParse(myString, System.Globalization.NumberStyles.Any, System.Globalization.CultureInfo.InvariantCulture, out thisReadDelay))
                            { //parse as a float
                        ...
                        case "<indent=": //set the indent to be here
                            float thisIndent;
                            if (float.TryParse(myString, System.Globalization.NumberStyles.Any, System.Globalization.CultureInfo.InvariantCulture, out thisIndent))
                            { //parse as a float
            ...
            { //stuff that gets reset!! single-use tags.
	...
	*/
    void RebuildTextInfo()
    {
        ...
        { //for each character to draw...
            ...
            {//drop a line
                if (i == 0)
                { //first character is a line break?
    ...
    void TrimCutoffText()
    {
        ...
        if (VerticalLimit > 0f && verticalLimitMode == VerticalLimitMode.CutOff)
        {
            ...
            {
                if (info[i].pos.y < -rawBottomRightBounds.y)
                                        ...
                                        verticalLimitMode == VerticalLimitMode.AutoPauseFull))
        {
            for (int i = 0; i < hyphenedText.Length; i++)
            {
                while (info[i].pos.y < autoPauseStopPoint)
                { //if this text is below the bounds... 
                    currentPauseCount++;
    ...
    { //this works!!! ahhhh!!!
        ...
        for (int i = 0, iL = lineBreaks.Count; i < iL; i++)
        {
            ...
            if (info[lineBreaks[i]].quadData != null)
            { //if last character is a quad...
        ...
        { //for each line of text //2016-06-09 new alignment script
            ...
            int spaceCount = 0;
            for (int j = rowStart, jL = lineBreaks[i] + 1; j < jL; j++)
	...
	*/
      ...
      //Mesh mesh = new Mesh();
        ...
        for (int i = 0, iL = hyphenedText.Length; i < iL; i++)
        {
            ...
            if (VerticalLimit > 0f && (verticalLimitMode == VerticalLimitMode.ShowLast ||
                                        ...
                                        verticalLimitMode == VerticalLimitMode.AutoPauseFull) &&
                lowestDrawnPosition < -rawBottomRightBounds.y)
            ...
            if (info[i].jitterData != null && !data.disableAnimatedText && !disableAnimatedText)
            ...
            if ((VerticalLimit > 0f && verticalLimitMode != VerticalLimitMode.Ignore) &&  //if vertical limit is on and not set to ignore...
                (info[i].pos.y + info[i].size + lowestLineOffset.y > -rawTopLeftBounds.y + 0.00001f/* || info[i].pos.y < -rawBottomRightBounds.y*/))
            ...
            if (info[i].textureData != null && (i != iL - 1 || (i == iL - 1 && info[i].TopRightVert != Vector3.zero)))
            { //not last character nothing!
                ...
                if (info[i].textureData.relativeToLetter)
            ...
            //Color data:
            if (info[i].quadData != null && !info[i].quadData.silhouette)
            ...
            else if (info[i].textureData != null)
            { //it has a texture
                endCol32[4 * i + 0] = Color.white; //set color to be white, so it doesn't interfere with texture
                endCol32[4 * i + 1] = Color.white;
                endCol32[4 * i + 2] = Color.white;
    ...
    { //0 == start mesh, < 0 == end mesh, > 0 == midway mesh
        if (textMesh == null)
        {
        ...
        if (text.Length > 0)
        {
            ...
            { //this also only needs to be set on Rebuild()
    ...
    void ApplyMaterials()
    { //turn submesh data into material data
      //do a check first to see if materials need to change
        ...
        if (uiMode)
        {//for now, simple way to disallow multiple materials on canvas, since it seems to cause a crash
         //2017-02-12 fixed it??
         //2017-04-14 FIXED IT
            if (this != null && t.gameObject.activeInHierarchy)
            { //prevents text from rendering weird
    ...
    SubMeshData DoesSubmeshExist(SuperTextMesh stm, STMTextInfo info)
    { //find a submesh that this character can exist on
        for (int i = 0, iL = subMeshes.Count; i < iL; i++)
        {
            bool safe = true;

            if (info.materialData != null)
            { //it has material data?
                if (subMeshes[i].refMat != info.materialData.material)
                { //if the two materials dont match
                    safe = false;
                }
            }
            else
            { //there's no material data on this letter, so compare to STM default
            ...
            { //there's texture data?
                if (subMeshes[i].refMask != info.textureData.texture)
                { //if the two textures dont match...
                  //return subMeshes[i];
            ...
            else
            {
                ...
                if (info.quadData == null && subMeshes[i].refMask != null)
                { //if this submesh has texture data, is not null too
                    safe = false;
            ...
            { //if it has quad data 
                if (subMeshes[i].refTex != info.quadData.texture)
                { //if the two textures aren't the same...
                    safe = false;
                }
                if ((subMeshes[i].refTex == subMeshes[i].refMask) == info.quadData.silhouette)
                { //if they're not both a silhouette
                    safe = false;
                }
            }
            else
            { //no quad data
                if (subMeshes[i].refTex != null)
                { //but the submesh does have it
                    safe = false;
                }
            }
...
{ //used internally by STM for keeping track of submeshes
  //public string name;
    ...
    public SubMeshData(SuperTextMesh stm, STMTextInfo info)
    { //from different data types
      //this.refMask = texData.texture;
        ...
        if (info.quadData != null && !info.quadData.silhouette)
        { //nah, use quad instead...
